...
class PerceptualLoss(nn.Module):
    ...
    def _freeze_weights(self):
        """Freeze all weights in the VGG model."""
    ...
    def _load_weights(self):
        ...
        weight_file = Path(os.path.join(os.path.dirname(__file__), '../metric_checkpoint/imagenet-vgg-verydeep-19.mat'))
        weight_file.parent.mkdir(exist_ok=True, parents=True)

        if not torch.distributed.is_initialized() or torch.distributed.get_rank() == 0:
            # Download weights if kneeded
