...
class SRNDataset():
    def __init__(self, cfg,
        ...
        if n_cond_imgs is not None and n_cond_imgs >= 3:
            self.no_imgs_per_example = n_cond_imgs + 1
            # idxs_pool = [64, 104, 240, 7, 22, 37, 52, 67, 82, 97, 144, 159, 174, 189, 204]
            idxs_pool = [80, 104, 198, 64, 1, 33]
            self.source_img_idxs = idxs_pool[:n_cond_imgs]
        elif convert_to_single_conditioning:
            self.no_imgs_per_example = 2
            self.source_img_idxs = [64]
            # self.source_img_idxs = [33]
        elif convert_to_double_conditioning:
            self.no_imgs_per_example = 3
            self.source_img_idxs = [64, 104]
        else:
            self.no_imgs_per_example = cfg.data.get("no_imgs_per_example", 4)
        ...
        if self.cfg.data.get('debug_mode', False):
            self.intrins = self.intrins[:10]
            print("Debug mode is on, only use 10 examples")
        ...
        if not hasattr(self.cfg.data, "use_occ"):
            self.cfg.data.use_occ = False
    ...
    def get_item_for_testing(self, index, N_virtual_views):
        ...
        if self.cfg.data.use_occ:
            test_aug_imgs, test_Rs, test_Ts, test_aug_occs = self.get_attributes_selected_sequence_and_frames(example_id,
                                                                           testing_frame_idxs,
                                                                           for_testing=True)
        else:
        ...
        cond_and_virtual_views = self.rearange_order_for_diffusion(attributes, example_id=example_id)
        ...
        cond_and_virtual_views["test_pose_embeds"] = test_aug_imgs[:, 3:, ...]
        if self.cfg.data.use_occ:
            cond_and_virtual_views["test_occs"] = test_aug_occs[:, :1, ...]
    ...
    def get_attributes_selected_sequence_and_frames(self, example_id, frame_idxs,
        ...
        for frame_idx in frame_idxs:
            ...
            if self.cfg.data.use_occ:
                occ = self.all_occs[example_id][frame_idx].unsqueeze(0)
                if self.cfg.data.input_size[0] != self.original_size and not for_testing:
                    occ = self.resize_transform(occ)
        ...
        if self.cfg.data.use_occ:
            aug_occ_in = torch.cat(aug_occ_in, dim=0)
            return aug_img_in, camera_Rs, camera_Ts, aug_occ_in
        return aug_img_in, camera_Rs, camera_Ts

    def rearange_order_for_diffusion(self, attributes, example_id=None):
        if self.cfg.data.use_occ:
            aug_img_in, camera_Rs, camera_Ts, aug_occ_in = attributes
        else:
        ...
        if self.dataset_name == "train":
            ...
            num_train_cond = self.cfg.data.get("num_train_cond", 1)
            x_in = aug_img_in[input_order[num_train_cond:], :3, ...]
            x_cond = aug_img_in[input_order[:num_train_cond], :3, ...]
            pose_embed = aug_img_in[input_order, 3:, ...]

            all_imgs_copy = training_imgs[input_order, ...] * 2 - 1
            if self.cfg.data.use_occ:
                training_occs = aug_occ_in[:-1, :1, ...]
                validation_occs = aug_occ_in[-1:, :1, ...]
                all_occs_copy = training_occs[input_order, ...]
        else:
            ...
            if self.cfg.data.use_occ:
                validation_occs = aug_occ_in[testing_idxs, :1, ...]
                training_occs = aug_occ_in[cond_idxs, :1, ...]
                all_occs_copy = aug_occ_in[input_order, :1, ...]
        ...
        if self.cfg.data.use_occ:
            ret["training_occs"] = all_occs_copy
            ret["validation_occs"] = validation_occs
        return ret
    ...
    def load_example_id(self, example_id, intrin_path,
        ...
        if self.cfg.data.use_occ:
            occ_paths = sorted(glob.glob(os.path.join(dir_path, "opc", "*")))
            assert len(rgb_paths) == len(occ_paths)

        if not hasattr(self, "all_rgbs"):
            self.all_rgbs = {}
            self.all_poses = {}
            self.all_above_0_z_ind = {}
        if self.cfg.data.use_occ and not hasattr(self, "all_occs"):
            self.all_occs = {}
        ...
        if example_id not in self.all_rgbs.keys():
            ...
            if idxs_to_load is not None:
                ...
                if self.cfg.data.use_occ:
                    occ_paths_load = [occ_paths[i] for i in idxs_to_load]
            else:
                rgb_paths_load = rgb_paths
                pose_paths_load = pose_paths
                if self.cfg.data.use_occ:
                    occ_paths_load = occ_paths
            ...
            for rgb_path, pose_path in zip(rgb_paths_load, pose_paths_load):
                ...
                if self.cfg.data.use_occ:
                    occ_path = rgb_path.split("/")
                    occ_path[-2] = "opc"
                    occ_path = '/'.join(occ_path)
                    occ = imageio.imread(occ_path)
                    occ = self.image_to_tensor(occ)[:1, ...]
                    self.all_occs[example_id].append(occ)
                path_idx += 1
            ...
            if self.cfg.data.use_occ:
                self.all_occs[example_id] = torch.stack(self.all_occs[example_id])
    ...
    def __getitem__(self, index):
        ...
        return self.rearange_order_for_diffusion(attributes, example_id=example_id)
